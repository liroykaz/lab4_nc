...
<div id="general_box">
    ...
    <div id="Navigation"><center><h3>Удаление студентов</h3></center></div>
    ...
    <div id="Form_1">
        <h3>Чтобы вывести студентов указанной группы, введите её номер и нажмите кнопку.</h3>
        <form action="/student-from-groups" method="Post">
            <table align="center">
                <tr><th colspan="3" align="center">Студенты группы</th></tr>
                <tr><td align="right">Номер группы</td><td align ="left"><input type = "text" name="number_of_group"/></td></tr>
                <tr><td></td><td colspan="1" align="center"><input type="submit" name="add_button" value="Вывести"/></td><td></td></tr>
            </table>
        </form>
    </div>
    ...
    <div id="Content">
        <table id="myTable">
            <tr><th>ID студента</th><th>Имя</th><th>Фамилия</th><th>Группа</th><th>Дата принятия</th></tr>
            <%
                request.setCharacterEncoding("UTF-8");
                List<Student> students = (List<Student>)request.getAttribute("data");
                for (int i=0; i< students.size();i++) {
            %>
            <tr><% out.println("<td>" + students.get(i).getId() + "</td>" + "<td>"+ students.get(i).getName() +"</td>"+ "<td>"+ students.get(i).getSurname() +"</td>"+ "<td>"+ students.get(i).getGroup() +"</td>"+ "<td>"+ students.get(i).getDate() +"</td>"); %>
                    <%}%>
        </table>
